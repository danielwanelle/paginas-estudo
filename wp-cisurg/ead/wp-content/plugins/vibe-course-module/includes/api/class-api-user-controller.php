			...
			if($item->post_type == 'unit'){
				...
				if($fetch_item){
					...
					if(($unit_type == 'play' || $unit_type == 'music-file-1') && ( false !== strpos( $item->post_content, '[' ))){
						...
						//for videotube
						preg_match_all( '/\[videotube\](.+)\[\/videotube]/', $item->post_content, $matches,PREG_SET_ORDER);
						if ( !empty( $matches ) ){
							global $wpdb;
							if(is_array($matches)){
								if(!empty($matches)){
									foreach ($matches as $key => $value) {
										if(!empty($value[1])){
											$iframes[] = array('shortcode'=>'videotube','value'=> $value[1]);
										}
									}
								}
							}	
						}
						$item->post_content = preg_replace ( '/\[videotube\](.+)\[\/videotube]/' , '' , $item->post_content );
						
						//for fvplayer video  
						preg_match_all( '/\[fvplayer id="(\d+)"\]/', $item->post_content, $matches,PREG_SET_ORDER);
						if ( !empty( $matches ) ){
							$srcs = array();
							global $wpdb;
							foreach ( $matches as $key=>$shortcode2 ) {
								if(!empty($shortcode2[1])){  // fvplayer id
									$query = "SELECT src FROM `{$wpdb->prefix}fv_player_videos` where id={$shortcode2[1]}";
									$src = $wpdb->get_results(  $query ,ARRAY_A);
									if(!empty($src)){
										$srcs[] =  $src;
									}
								}
							}
							if(!empty($srcs) && is_array($srcs)){
								foreach($srcs as $key=>$value){
									if(!empty($value[0]) && !empty($value[0]['src'])){
										$iframes[] = array('shortcode'=>'fvplayer','value'=>$value[0]['src']);
									}
								}
							}	
						}
						// remove fvplayer shortcode
						$item->post_content = preg_replace ( '/\[fvplayer id="(\d+)"\]/' , '' , $item->post_content );
						
						//for wplms bigbluebutton 
						preg_match_all( '/\[wplms_bbb token=\'(\d+)\'\]\[\/wplms_bbb]/', $item->post_content, $matches_bbb,PREG_SET_ORDER);
						if ( !empty( $matches_bbb ) && class_exists('Wplms_Bbb') && class_exists('Bigbluebutton_Api')){
							foreach($matches_bbb as $key=>$shortcode2){
								if(!empty($shortcode2[1])){  // meeting_id/token
									$meeting_id = $shortcode2[1];

									$bbb_obj = Wplms_Bbb::init();
									$meeting = $bbb_obj->get_meeting($meeting_id);
									$user_id = $this->user_id;
									$users = $bbb_obj->users_from_restriction( $meeting,1);
									
									if(!empty($meeting ) && !empty($users) && in_array($user_id , $users)){
										...
										if($meeting_expiry_time <= time()){
											...
											$iframes[] = array('shortcode'=>'wplms_bbb','status'=>0,'value'=>__('Meeting is expired','vibe'));
										}else{
											...
											if( $start_time < time()){ //show meeting iframe   
												...
												$attendeepassword = get_post_meta($meeting_id ,'bbb-room-viewer-code',true);
												$moderatorpw = get_post_meta($meeting_id ,'bbb-room-moderator-code',true);
												$is_recorded = get_post_meta($meeting_id ,'bbb-room-recordable',true);
												$waitfor_moderator = get_post_meta($meeting_id ,'bbb-room-wait-for-moderator',true);
												$check_author_admin_for_meeting = 0;
												if($meeting_post->post_author == $user->ID || user_can($user,'manage_options')){
													$check_author_admin_for_meeting = 1;
													$password = $moderatorpw;
												}else{
													$password = $attendeepassword;
												}
												if (  $check_author_admin_for_meeting || !$waitfor_moderator  || Bigbluebutton_Api::is_meeting_running($meeting_id )) {
													//If the password submitted is correct then the user gets redirected
												
													$bigbluebutton_joinURL = Bigbluebutton_Api::get_join_meeting_url( $meeting_id , (!empty($user->display_name)?$user->display_name:(!empty($user->user_login)?$user->user_login:$user->user_email)), $password, site_url() );
													
													if(!empty($bigbluebutton_joinURL)){
														if(empty($iframes)){
															$iframes = array();
														}
														$iframes[] = array('shortcode'=>'wplms_bbb','status'=>1,'value'=>$bigbluebutton_joinURL);
													}
												}else{
													$iframes[] = array('shortcode'=>'wplms_bbb','status'=>0,'value'=>__('Moderator has not joined meeting yet','vibe'));
												}
			
											}
										}
									}else{
										$iframes[] = array('shortcode'=>'wplms_bbb','status'=>0,'value'=>__('Units content that not access','vibe'));
									}
								}	
							}
						}
						// remove  wplms bigbluebutton  shortcode
						$item->post_content = preg_replace ( '/\[wplms_bbb token=\'(\d+)\'\]\[\/wplms_bbb]/' , '' , $item->post_content );
					...
					if($course_status < 3 ){
						...
						if(empty($done_flag)){
							...
							if(apply_filters('course_status_item_bp_course_update_user_unit_completion_time',true)){
								$args = array(
           ...
           **/
            ...
            if(!empty($attachment_id[0])  ){
                  ...
                  require_once(ABSPATH . 'wp-admin' . '/includes/media.php');
					...
					/* After assignment uploaded successfully */ 
					do_action('After_assignment_upload_api',$request);
            } 
